...
{
    for(int i = 0; i < word.size(); i++)                // Neste for é definido p1 a p8. No vector de paridade é feito uma soma sucessiva fazendo um AND
        for(int j = PARITYMAX-1; j>=0; j--)             // do bit da palavra com uma tabela de binários que indica se aquela posição faz parte de cada bit de paridade
            parityBit[-1*(j-PARITYMAX+1)] = parityBit[-1*(j-PARITYMAX+1)]+(matriz[i+1][j]*word[i]); //Matriz [bit] [[0][1][2][3]]<- Pos. do bit. Ex Matriz[3][3] = 1 ja que [0][0][1][1]
...
{
    int erro = 0, erroP = 0;
    erroP = doubleBitVerify(wordError);
...
void CRC10(vector<bool> &wordCRC, int verify, vector<bool> &word)
{
    ...
    vector<bool> CRC = {0,0,0,0};
                                                        // CRC que primeiro será o resto, para ao fim virar o CRC
    if(verify==0)
    ...
    int i = CRC.size();                                 // Valor padrão igual 4;

    while(i<wordCRC.size()+1)                           //começa no i = tamanho do CRC pois essa será a primeira divisão
    {
        cout<<endl<<"CRC ^ polinômio: "<<CRC[0]<<CRC[1]<<CRC[2]<<CRC[3]<<"^"<<protectPolynom[0]<<protectPolynom[1]<<protectPolynom[2]<<protectPolynom[3]<<":";
        if(CRC[0] != 0)
        {
            for(int k = 0; k<CRC.size(); k++)
                CRC[k] = CRC[k] ^ protectPolynom[k];

            cout<<CRC[0]<<CRC[1]<<CRC[2]<<CRC[3]<<endl;
        }
        else
        {
            if(i!=wordCRC.size())                       // Se tiver na última posição não pega o próximo valor
            {
                CRC.erase(CRC.begin());                 // remover o primeiro que é zero
...
int main()
{
    ...
    while(true)
    {
        ...
        if(mode == 1)
        {
            ...
            if(mode == 1)                             // Modo = 1 receptor recebe a mensagem e verifica erro
            {
                cin.sync();
        ...
        if(mode == 2)
        {
            ...
            for(int i = 0; i<4; i++)
                word.insert(word.begin()+pow(2,i)-1,0);
